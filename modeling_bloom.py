...
def _prepare_4d_causal_attention_mask_with_cache_position(
    ...
    if attention_mask is not None and attention_mask.dim() == 4:
        ...
        causal_mask *= torch.arange(target_length, device=device) > g.reshape(-1, 1)
...
class BloomAttention(nn.Module):
    ...
    def _merge_heads(self, x: torch.Tensor) -> torch.Tensor:
        """
        Merge heads together over the last dimensiong
...
class BloomModel(BloomPreTrainedModel):
    ...
    def forward(
        ...
        **deprecated_arguments,
        ...
        # )
        causal_mask = _prepare_4d_attention_mask(attention_mask, dtype=inputs_embeds.dtype, tgt_len=seq_length)
        # causal_mask = None
